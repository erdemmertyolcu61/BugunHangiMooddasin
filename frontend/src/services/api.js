...
// ─── Couch Mode (Birlikte İzle) ───

export async function couchCreate() {
  const res = await fetch(`${BASE}/couch/create`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.detail || `Oda oluşturulamadı (${res.status})`);
  }
  return res.json();
}

export async function couchJoin(roomCode) {
  const res = await fetch(`${BASE}/couch/join`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.detail || `Odaya katılınamadı (${res.status})`);
  }
  return res.json();
}

export async function couchStatus(roomCode) {
  const res = await fetch(`${BASE}/couch/${encodeURIComponent(roomCode)}`, {
    headers: authHeaders(),
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.detail || `Oda durumu alınamadı (${res.status})`);
  }
  return res.json();
}

export async function couchSelectMood(roomCode, moodId) {
  const res = await fetch(`${BASE}/couch/${encodeURIComponent(roomCode)}/mood`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.detail || `Mood seçilemedi (${res.status})`);
  }
  return res.json();
}

export async function couchMovies(roomCode, page = 1) {
  const res = await fetch(`${BASE}/couch/${encodeURIComponent(roomCode)}/movies?page=${page}`, {
    headers: authHeaders(),
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.detail || `Filmler alınamadı (${res.status})`);
  }
  return res.json();
}

export async function couchClose(roomCode) {
  const res = await fetch(`${BASE}/couch/${encodeURIComponent(roomCode)}/close`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.detail || `Oda kapatılamadı (${res.status})`);
  }
  return res.json();
}

export async function couchLeave(roomCode) {
  const res = await fetch(`${BASE}/couch/${encodeURIComponent(roomCode)}/leave`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.detail || `Odadan ayrılınamadı (${res.status})`);
  }
  return res.json();
}
