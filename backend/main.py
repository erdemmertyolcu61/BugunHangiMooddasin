...
# ─── Couch Mode (Grup Seansı / Ne İzleyelim Odası) ───

import string as _string
import random as _couch_rnd

def _generate_room_code() -> str:
    """6 karakterlik benzersiz oda kodu: SM-XXXX (harf+rakam)."""
    chars = _string.ascii_uppercase + _string.digits
    suffix = ''.join(_couch_rnd.choices(chars, k=4))
    return f"SM-{suffix}"

# Üstad — grup modu satırları
_COUCH_USTAD_LINES = [
    "Bu akşam seans ortaklaşa — Üstad ikiniz için özenle seçti.",
    "Evlatlar, bu gece ekran sizin; Üstad yalnızca yol gösterir.",
    "Koltuğa gömülün, bu akşamki seansınız için arşivi taradım.",
    "İki sinefil, bir ekran, bir gece — işte Üstad'ın seçimi.",
    "Beraber izlemek ayrı bir ritüeldir. Bu geceye yakışan filmler hazır.",
    "Grup seansı başlasın — bu sefer Üstad herkesi düşündü.",
    "Perde açılsın, evlatlar. Bu akşam arşivin en iyileri sizinle.",
    "Ortaklaşa bir gece, ortaklaşa bir ruh hali — Üstad hazır.",
]

class CouchCreateRequest(BaseModel):
    pass  # sadece auth gerekli

class CouchJoinRequest(BaseModel):
    room_code: str

class CouchMoodRequest(BaseModel):
    mood_id: str

class CouchKickRequest(BaseModel):
    user_id: int


@app.post("/api/couch/create")
async def couch_create_room(user=Depends(verify_user)):
    """Oda kur — giriş yapmış kullanıcı host olur, 6 haneli kod döner."""
    user_id = user.get("user_id", 0)
    if not user_id:
        raise HTTPException(status_code=401, detail="Giriş gerekli")

    # Kullanıcının zaten aktif odası var mı?
    async with _db_conn(cache.db_path, user_data=True) as db:
        cur = await db.execute(
            "SELECT room_code FROM couch_rooms WHERE host_user_id = ? AND status IN ('waiting', 'active')",
            (user_id,)
        )
        existing = await cur.fetchone()
        if existing:
            # Mevcut odayı döndür (tekrar oluşturma)
            code = existing[0]
            return {"room_code": code, "status": "existing", "message": "Zaten aktif bir odan var."}

        # Yeni oda kodu üret (çakışma kontrolü)
        for _ in range(20):
            code = _generate_room_code()
            cur2 = await db.execute("SELECT 1 FROM couch_rooms WHERE room_code = ?", (code,))
            if not await cur2.fetchone():
                break
        else:
            raise HTTPException(status_code=500, detail="Oda kodu üretilemedi, tekrar deneyin.")

        expires_at = (datetime.utcnow() + timedelta(hours=4)).isoformat()
        await db.execute("""
            INSERT INTO couch_rooms (room_code, host_user_id, status, expires_at)
            VALUES (?, ?, 'waiting', ?)
        """, (code, user_id, expires_at))
        await db.commit()

    return {"room_code": code, "status": "created", "message": "Oda kuruldu! Kodu arkadaşınla paylaş."}


@app.post("/api/couch/join")
async def couch_join_room(req: CouchJoinRequest, user=Depends(verify_user)):
    """Odaya katıl — kod ile mevcut bir odaya giriş yap."""
    user_id = user.get("user_id", 0)
    if not user_id:
        raise HTTPException(status_code=401, detail="Giriş gerekli")
    ...
    if not code:
        raise HTTPException(status_code=400, detail="Oda kodu gerekli")

    async with _db_conn(cache.db_path, user_data=True) as db:
        cur = await db.execute(
            "SELECT host_user_id, guest_user_id, status, expires_at FROM couch_rooms WHERE room_code = ?",
            (code,)
        )
        room = await cur.fetchone()
        if not room:
            raise HTTPException(status_code=404, detail="Oda bulunamadı. Kodu kontrol et.")

        host_id, guest_id, status, expires_at = room
        ...
        if expires_at:
            ...
            try:
                ...
                if _dt.utcnow() > exp:
                    await db.execute("UPDATE couch_rooms SET status = 'expired' WHERE room_code = ?", (code,))
                    await db.commit()
                    raise HTTPException(status_code=410, detail="Bu odanın süresi dolmuş.")
            except (ValueError, TypeError):
                pass

        if status not in ("waiting", "active"):
            raise HTTPException(status_code=410, detail="Bu oda artık aktif değil.")

        # Host kendisi katılamaz
        if host_id == user_id:
            raise HTTPException(status_code=400, detail="Kendi odana katılamazsın — zaten host'sun!")

        # Zaten bir guest var mı?
        if guest_id and guest_id != user_id:
            raise HTTPException(status_code=409, detail="Bu oda zaten dolu.")

        # Guest olarak ekle
        await db.execute("""
            UPDATE couch_rooms SET guest_user_id = ?, status = 'active'
            WHERE room_code = ?
        """, (user_id, code))
        await db.commit()

    return {"room_code": code, "status": "joined", "message": "Odaya katıldın! Birlikte mood seçebilirsiniz."}


@app.get("/api/couch/{room_code}")
async def couch_room_status(room_code: str, user=Depends(verify_user)):
    """Oda durumunu sorgula — kim var, mood seçildi mi, filmler hazır mı."""
    user_id = user.get("user_id", 0)
    code = room_code.strip().upper()

    async with _db_conn(cache.db_path, user_data=True) as db:
        cur = await db.execute(
            "SELECT host_user_id, guest_user_id, selected_mood, status, created_at, expires_at FROM couch_rooms WHERE room_code = ?",
            (code,)
        )
        room = await cur.fetchone()
        if not room:
        ...
        host_id, guest_id, selected_mood, status, created_at, expires_at = room

        # Sadece odadaki kullanıcılar görebilir
        if user_id not in (host_id, guest_id):
            raise HTTPException(status_code=403, detail="Bu odaya erişim yetkiniz yok.")

        # Kullanıcı bilgilerini al
        members = []
        for uid in [host_id, guest_id]:
            if uid:
                cur2 = await db.execute("SELECT id, name, picture FROM users WHERE id = ?", (uid,))
                u = await cur2.fetchone()
                if u:
                    members.append({"user_id": u[0], "name": u[1], "picture": u[2], "role": "host" if uid == host_id else "guest"})
...
@app.post("/api/couch/{room_code}/mood")
async def couch_select_mood(room_code: str, req: CouchMoodRequest, user=Depends(verify_user)):
    """Oda için mood seç — sadece odadaki kullanıcılar seçebilir."""
    user_id = user.get("user_id", 0)
    code = room_code.strip().upper()
    mood_id = req.mood_id.strip()

    # Mood geçerli mi?
    if mood_id not in MOOD_NAMES:
        raise HTTPException(status_code=400, detail=f"'{mood_id}' geçerli bir mood değil.")

    async with _db_conn(cache.db_path, user_data=True) as db:
        cur = await db.execute(
            "SELECT host_user_id, guest_user_id, status FROM couch_rooms WHERE room_code = ?",
            (code,)
        )
        room = await cur.fetchone()
        if not room:
        ...
        await db.execute(
            "UPDATE couch_rooms SET selected_mood = ? WHERE room_code = ?",
            (mood_id, code)
        )
        await db.commit()
    ...
    return {
        "room_code": code,
        "selected_mood": mood_id,
        "mood_name": MOOD_NAMES.get(mood_id, mood_id),
        "message": f"Mood seçildi: {MOOD_NAMES.get(mood_id, mood_id)}. Filmler hazırlanıyor!",
    }


@app.get("/api/couch/{room_code}/movies")
async def couch_get_movies(
    room_code: str,
    page: int = Query(1, ge=1),
    user=Depends(verify_user),
):
    """
    Couch Mode film listesi — odanın seçili mood'u üzerinden
    mevcut repository motorunu kullanır. Üstad notu grup moduna uyarlanır.
    Claude API çağrısı YOKTUR — tamamen kural tabanlı.
    """
    user_id = user.get("user_id", 0)
    code = room_code.strip().upper()
    ...
    async with _db_conn(cache.db_path, user_data=True) as db:
        cur = await db.execute(
            "SELECT host_user_id, guest_user_id, selected_mood, status FROM couch_rooms WHERE room_code = ?",
            (code,)
        )
        room = await cur.fetchone()
        if not room:
        ...
        host_id, guest_id, selected_mood, status = room
        if user_id not in (host_id, guest_id):
            raise HTTPException(status_code=403, detail="Bu odaya erişim yetkiniz yok.")
        if not selected_mood:
            raise HTTPException(status_code=400, detail="Henüz mood seçilmedi. Önce bir mood belirleyin.")

    # Mevcut repository motorunu kullan (get_repository_movies ile aynı mantık)
    from backend.mood_profiles import MOOD_GENRE_MAP, get_tmdb_params
    from backend.mood_scoring import calculate_mood_scores

    config = MOOD_GENRE_MAP.get(selected_mood)
    if not config:
    ...
    # Batch enrich (aynı pattern: ana repository endpoint'i gibi)
    if page_movies:
        ...
        for movie in page_movies:
            ...
            movie["ai_classified"] = classifications_map.get(mid) == selected_mood
            movie["primaryMoods"] = []
    ...
    # Grup modu üstad satırı
    ustad_line = _couch_rnd.choice(_COUCH_USTAD_LINES)

    return {
        "room_code": code,
        "selected_mood": selected_mood,
        "mood_name": MOOD_NAMES.get(selected_mood, selected_mood),
        "ustad_notu": ustad_line,
        "movies": page_movies,
        "page": page,
...
@app.post("/api/couch/{room_code}/close")
async def couch_close_room(room_code: str, user=Depends(verify_user)):
    """Odayı kapat — sadece host kapatabilir."""
    user_id = user.get("user_id", 0)
    code = room_code.strip().upper()

    async with _db_conn(cache.db_path, user_data=True) as db:
        cur = await db.execute(
            "SELECT host_user_id, status FROM couch_rooms WHERE room_code = ?",
            (code,)
        )
        room = await cur.fetchone()
        if not room:
            raise HTTPException(status_code=404, detail="Oda bulunamadı.")
        if room[0] != user_id:
            raise HTTPException(status_code=403, detail="Sadece oda sahibi odayı kapatabilir.")
        if room[1] == "closed":
            return {"room_code": code, "status": "closed", "message": "Oda zaten kapalı."}

        await db.execute("UPDATE couch_rooms SET status = 'closed' WHERE room_code = ?", (code,))
        await db.commit()

    return {"room_code": code, "status": "closed", "message": "Oda kapatıldı. İyi seyirler!"}


@app.post("/api/couch/{room_code}/leave")
async def couch_leave_room(room_code: str, user=Depends(verify_user)):
    """Odadan ayrıl — guest ayrılırsa oda 'waiting'e döner, host ayrılırsa oda kapanır."""
    user_id = user.get("user_id", 0)
    code = room_code.strip().upper()
    ...
    async with _db_conn(cache.db_path, user_data=True) as db:
        ...
        if user_id == host_id:
            # Host ayrılırsa oda kapanır
            await db.execute("UPDATE couch_rooms SET status = 'closed' WHERE room_code = ?", (code,))
            await db.commit()
            return {"room_code": code, "status": "closed", "message": "Host ayrıldı — oda kapatıldı."}
        else:
            # Guest ayrılırsa oda beklemeye döner
            await db.execute(
                "UPDATE couch_rooms SET guest_user_id = NULL, status = 'waiting' WHERE room_code = ?",
                (code,)
            )
            await db.commit()
            return {"room_code": code, "status": "waiting", "message": "Odadan ayrıldın."}
