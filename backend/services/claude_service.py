...
class ClaudeService:
    ...
    async def analyze_movie(self, title: str, overview: str, ratings: dict,
        ...
        prompt = (
            ...
            "- MUTLAKA filmin anlatısındaki SOMUT bir unsura değin — bir karakter dinamiği, bir mekânın dramatik işlevi, bir sahnenin yarattığı ruh hali, bir diyaloğun altında yatan gerilim. Genel övgü yerine 'bu filmde X unsuru Y etkisini yaratıyor' türünden somut gözlem.\n"
            ...
            "- PLOT-DRIVEN: Yukarıdaki 'Konu' metnini DERİNLEMESİNE analiz et. Özeti tekrarlama ama anlatıdaki tematik çatışmayı (yalnızlık/özgürlük, suç/ceza, aşk/fedakarlık vb.), ana karakterin iç yolculuğunu veya filmin sorduğu varoluşsal soruyu tespit et ve notuna entegre et. Yüzeysel 'harika hikaye' DEĞİL — 'bu hikaye aslında X hakkında' entelektüel okuma.\n\n"
            "Örnek iyi notlar (entelektüel ton + plot-driven):\n"
            ...
            "- 'Üstadın Notu: Bong Joon-ho, sınıf çatışmasını dikey bir mimariye kodluyor — merdiven, Eisenstein'ın diyalektik kurgusunun mekânsal hâli. Bodrum katındaki ailenin yukarıya tırmanışı, toplumsal hiyerarşinin fiziksel bir metaforu. Tür sinemasını toplumsal eleştiriyle aynı karede eritmesi onu çağımızın en politik biçimcisi yapıyor.'\n"
            ...
            "- 'Üstadın Notu: Nolan, uzay-zamanı bir baba-kız hikayesinin duygusal çekim alanına yerleştiriyor — Cooper'ın kara delik ötesindeki o raf sahnesi, bilimi şiire dönüştüren ender anlardan. Kip Thorne'un fiziği Kubrick'in soğuk estetiğini değil, Spielberg'in sıcak hümanizmini giydiriyor.'\n\n"
